...
> **Status:** Phase 5A complete — **Pub/Sub**: `SUBSCRIBE`/`PSUBSCRIBE`/`PUBLISH`/
> `PUBSUB`, Redis 7 sharded pub/sub (`SSUBSCRIBE`/`SPUBLISH`), the RESP3 push type,
> and RESP2 subscriber-mode restrictions — verified against real `redis-cli`. On top
> of **AOF + RDB persistence** (multi-part AOF with all three `appendfsync` policies,
> fork-free `BGREWRITEAOF`/`BGSAVE`, cross-compatible RDB), all five data types, the
> `SCAN` family, two-tier expiration, memory accounting, and `maxmemory` eviction.
> Next (Phase 5): transactions (MULTI/EXEC/WATCH), blocking commands, and Lua
> scripting.
